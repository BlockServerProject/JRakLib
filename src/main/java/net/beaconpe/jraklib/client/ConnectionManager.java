...
public class ConnectionManager
{
    ...
    private boolean connect(int payloadSize, int packets) throws IOException
    {
        for (int i = 0; i < packets; i++)
        {
            ...
            if (response != null && response.getData()[0] == OPEN_CONNECTION_REPLY_1.ID)
            {
                ...
                packet.buffer = wrappedBuffer(response.getData());
    ...
    private boolean receivePacket() throws IOException
    {
        ...
        if (len > 0)
        {
            ...
            if (pkt != null)
            {
                pkt.buffer = wrappedBuffer(packet.getData());
    ...
    public void streamEncapsulated(EncapsulatedPacket packet, byte flags)
    {
        ...
        client.pushThreadToMainPacket(bb);
    }
    ...
    protected void streamOpen(long serverId)
    {
        ...
        ByteBuf bb = buffer(10 + identifier.getBytes().length);
        bb.writeByte(JRakLib.PACKET_OPEN_SESSION).writeByte((byte) identifier.getBytes().length).writeBytes(identifier.getBytes()).writeBytes(Binary.writeLong(serverId));
        client.pushThreadToMainPacket(bb);
    }
    ...
    {
        ByteBuf packet = client.readMainToThreadPacket();
        if (packet == null)
        {
            return false;
        }
