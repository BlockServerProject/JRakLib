...
public class Session
{
    ...
    public Session(SessionManager sessionManager, ChannelHandlerContext ctx, String address, int port)
    {
    ...
    {
        if (!isActive && (lastUpdate + 10000) < time)
        { //10 second timeout
            disconnect("timeout");
            return;
        }
        ...
        for (Integer seq : recoveryQueue.keySet())
        {
            ...
            if (pk.sendTime < System.currentTimeMillis() - 6000)
            { //If no ACK in 6 seconds, resend :)
                packetToSend.add(pk);
    ...
    {
        sessionManager.sendPacket(packet, ctx, address, port);
    }
    ...
    private void addToQueue(EncapsulatedPacket pk, int flags) throws IOException
    {
        ...
        if (priority == JRakLib.PRIORITY_IMMEDIATE)
        { //Skip queues
    ...
    public void addEncapsulatedToQueue(EncapsulatedPacket packet, byte flags) throws IOException
    {
        ...
        {
            needACK.put(packet.identifierACK, new ConcurrentHashMap<Integer, Integer>());
        }
        ...
        {
            byte[][] buffers = Binary.splitbytes(packet.buffer.copy(), mtuSize - 34);
            int splitID = this.splitID++;
            splitID = splitID % 65536;
            for (int count = 0; count < buffers.length; count++)
            {
                ByteBuf buffer = wrappedBuffer(buffers[count]);
    ...
    private void handleSplit(EncapsulatedPacket packet) throws IOException
    {
        ...
        if (splitPackets.get(packet.splitID).values().size() == packet.splitCount)
        {
            ...
            {
                bb.writeBytes(splitPackets.get(packet.splitID).get(i).buffer);
            }
            pk.buffer = bb.copy();
            bb = null;
            pk.length = pk.buffer.readableBytes();
    ...
    private void handleEncapsulatedPacketRoute(EncapsulatedPacket packet) throws IOException
    {
        ...
        byte id = packet.buffer.getByte(0);
        if (id < 0x80)
    ...
    {
        if ((packet.getID() == 1) || (packet.getID() == 5))
        {
            this.resetSession();
        }
        isActive = true;
        ...
        {
            if (packet.buffer.getByte(0) >= 0x80 || packet.buffer.getByte(0) <= 0x8f && packet instanceof DataPacket)
            {
                packet.decode();
        ...
        } else if (packet.buffer.getByte(0) > 0x00 || packet.buffer.getByte(0) < 0x80)
        { //Not Data packet :)
            packet.decode();
            if (packet instanceof UNCONNECTED_PING)
            {
                ...
                {
                    pk.serverName = "MCPE;JRakLib;34;0.12.1;0;1";
                } else
                {
    ...
    {
        ByteBuf data = wrappedBuffer(new byte[]
        {
            0x00, 0x00, 0x08, 0x15
        });
